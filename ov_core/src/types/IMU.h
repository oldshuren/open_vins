    ...
    public:
        ...
        */
        ...
        std::shared_ptr<Type> clone_component(int clone_size) override {
            assert(clone_size == 0 || clone_size == 3 || clone_size ==  6 || clone_size == 15);

            std::shared_ptr<Type> Clone;

            if(clone_size == 3){
                Clone = std::shared_ptr<Type>(new JPLQuat());
            }else if(clone_size == 6){
                Clone = std::shared_ptr<Type>(new PoseJPL());
            }else {
                return clone();
            }
        ...
        /// Pose type access
        std::shared_ptr<PoseJPL> pose() const {
            return _pose;
        }
        ...
        /// Pose23 access
        Eigen::Matrix<double, 5, 5> pose_se23(bool rot_flip) const {
            Eigen::Matrix<double, 5, 5> pose_23 = Eigen::Matrix<double,5,5>::Identity();
            pose_23.block<4,4>(0,0) = pose()->pose_se3(rot_flip);
            pose_23.block<3,1>(0,4) = vel();
            return pose_23;
        }

        Eigen::Matrix<double, 5, 5> pose_se23_fej(bool rot_flip) const {
            Eigen::Matrix<double, 5, 5> pose_23 = Eigen::Matrix<double,5,5>::Identity();
            pose_23.block<4,4>(0,0) = pose()->pose_se3_fej(rot_flip);
            pose_23.block<3,1>(0,4) = vel_fej();
            return pose_23;
        }
